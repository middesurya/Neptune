...
const navLinks = [
  { href: '/', label: 'Home' },
  { href: '/triquetra', label: 'Triquetra' },
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            <svg 
              viewBox="0 0 100 100" 
              className="w-8 h-8 text-[var(--accent-amber)]"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
            >
            ...
            <span className="font-[var(--font-orbitron)] font-bold text-lg tracking-wider">
              NEPTUNE
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-[var(--text-secondary)] hover:text-[var(--accent-amber)] transition-colors text-sm tracking-wider uppercase font-mono"
              >
                {link.label}
              </Link>
            ))}
            ...
            <div className="px-6 py-4 flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="text-[var(--text-secondary)] hover:text-[var(--accent-amber)] transition-colors text-sm tracking-wider uppercase font-mono py-2"
                >
                  {link.label}
                </Link>
              ))}
